...
class DocumentCheckpointingExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController? {
        ...
        checkpointObserver = NotificationCenter.default.addObserver(forName: .PSPDFDocumentCheckpointSaved, object: document.checkpointer, queue: OperationQueue.main) { [weak self] _ in
            if let observer = self?.checkpointObserver {
