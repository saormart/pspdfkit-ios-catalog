...
class InstantDocumentManager: NSObject, InstantClientDelegate {
    ...
    private class ManagedDocument: NSObject {
        let serverURL: URL
        var JWT: String
        let documentIdentifier: InstantExampleDocumentIdentifier
        ...
        init(documentIdentifier: InstantExampleDocumentIdentifier, encodedDocumentId: String, url: URL, serverURL: URL, JWT: String, observer: InstantDocumentManagerDocumentObserver? = nil, client: InstantClient, descriptor: InstantDocumentDescriptor) {
    ...
    /// Changes the default group set of the newly created annotations for the document backed by
    /// the given `InstantDocumentInfo`
    func changeDefaultGroup(_ defaultGroup: String, for documentInfo: InstantDocumentInfo) throws {
        let descriptor = try documentDescriptor(for: documentInfo)
        try descriptor.overrideDefaultGroup(with: defaultGroup)
    }
