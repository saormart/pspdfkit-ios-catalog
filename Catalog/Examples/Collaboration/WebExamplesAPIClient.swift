...
public class WebExamplesAPIClient: NSObject, URLSessionTaskDelegate {
    ...
    struct Endpoint {
        ...
        static let documentWithLayers = Endpoint(document: .constructionPlan)
    }
    ...
    /// The `jwtParameters` are optional but when specified they are added to the JWT generated for
    /// accessing the document of the created session.
    /// The completion handler may be called on a background thread.
    public func createNewSession(documentIdentifier: InstantExampleDocumentIdentifier, jwtParameters: [String: Any]? = nil, completion: @escaping (Result<InstantDocumentInfo, WebExamplesAPIClientError>) -> Void) {
        var request = authorizedRequest(forEndpoint: Endpoint(document: documentIdentifier))
        request.httpMethod = "POST"

        if let jwtParams = jwtParameters {
            let actualParams = ["jwtParameters": jwtParams]
            do {
                request.httpBody = try JSONSerialization.data(withJSONObject: actualParams)
            } catch let err {
                completion(.failure(.invalidRequest(underlying: err)))
            }
        }
...
extension WebExamplesAPIClient.Endpoint {
    init(document: InstantExampleDocumentIdentifier) {
        self.init(rawValue: document.rawValue)
    }
}
...
public enum WebExamplesAPIClientError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidRequest(let underlying?):
            return "The request is invalid. Reason: \(underlying)."
        case .invalidRequest(underlying: .none):
